class nailgun::cobbler(
  ...
  case $production {
    'prod', 'docker': {
      ...
      file { "/var/lib/cobbler/kickstarts/rhel-x86_64.ks":
      ...
      cobbler_distro { "rhel-x86_64":
        kernel => "${repo_root}/redhat/x86_64/isolinux/vmlinuz",
        initrd => "${repo_root}/redhat/x86_64/isolinux/initrd.img",
        arch => "x86_64",
        breed => "redhat",
        osversion => "rhel6",
        ksmeta => "tree=http://@@server@@:8080/redhat/x86_64/",
        require => Class["::cobbler::server"],
      }
      ...
      cobbler_profile { "rhel-x86_64":
        kickstart => "/var/lib/cobbler/kickstarts/rhel-x86_64.ks",
        kopts => "biosdevname=0 sshd=1 dhcptimeout=120",
        distro => "rhel-x86_64",
        ksmeta => "",
        menu => true,
